...
AudioEngine.prototype.initAudios = function (audio_files, path, config) {
	...
	for (let keyfile in audio_files) {
		...
		let objectHowl = {
			...
			onloaderror: function (what, is) {
				console.log("[onloaderror] " + this._src + " " + what, is);
			},
			onload: function () {
...
AudioEngine.prototype.processVolume = function (volume_input, min = 0, max = 1) {
	var new_volume = volume_input / (min + max);
	new_volume = Math.round(new_volume * 100) / 100;
	return Howler.volume(new_volume);
};

AudioEngine.prototype.removeKeyPlaying = function (key) {
	while (this.audioKeyPlaying.indexOf(key) > -1) {
		this.audioKeyPlaying.splice(this.audioKeyPlaying.indexOf(key), 1);
	}
};
...
AudioEngine.prototype.mute = function (mute) {
	Howler.mute(mute);
};
